	...
	export default {
		...
		methods: {
			...
			toPage(index, obj) {
				if (index == 0) {
					uni.navigateTo({
						url: '/pages/repository/photo?obj=' + JSON.stringify(obj)
					})
				} else if (index == 1) {
					uni.navigateTo({
						url: '/pages/repository/document?obj=' + JSON.stringify(obj)
					})
				} else if (index == 2) {
					uni.navigateTo({
						url: '/pages/repository/website?obj=' + JSON.stringify(obj)
					})
				} else if (index == 3) {
					uni.switchTab({
						url: '/pages/repository/repository?obj=' + JSON.stringify(obj)
					})
				}
			},
			editGroup(item) {
