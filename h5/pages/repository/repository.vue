					...
					<view class="card" v-if="curr<displayLimit" v-for="val,curr in item.values" :key="curr">
						...
						<view class="right">
							<view class="right_top">
								...
								<view class="icon">
									<image :src="edit" class="icon_l" @click="toAddAgent(1,val)"></image>
	...
	export default {
		...
		methods: {
			...
			toAddAgent(num,val) {
				...
				if (num==1) {
					uni.navigateTo({
						url: '/pages/repository/add_agent?val=' + JSON.stringify(val)
					})
				} else {
					uni.navigateTo({
						url: '/pages/repository/add_agent'
					})
				}
			},
			toDocument() {
				uni.navigateTo({
					url: '/pages/repository/document'
				})
			},
			toWebsite() {
				uni.navigateTo({
					url: '/pages/repository/website'
				})
			},
			toPhoto() {
				uni.navigateTo({
					url: '/pages/repository/photo'
				})
			},
			toAgent(item) {
				uni.navigateTo({
					url: '/pages/repository/agent?item=' + JSON.stringify(item)
				})
			},
			toGroup() {
